...
def get_placement_user(request, format=None):
    ...
    return Response({'response' : [{ 'placement_info_id' :e.placement_id, 'offers' : e.offers , 'month' : e.visit_month ,'placement_status' : e.placement_status_id,'school_id':e.school_id.school_id,'school_name':e.school_id.school_name, 'profile_ctc' : e.profile_ctc.split(' # ') if e.profile_ctc is not None else e.profile_ctc } for e in placement_details]}, status=status.HTTP_200_OK)