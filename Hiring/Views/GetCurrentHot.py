...
def get_current_hot(request, format=None):
    user = request.user
    batch = request.GET.get("batch",None)
    assigned = Assign_company.objects.filter(user=user)
    companies = [ e.comp_id for e in assigned ]
    placement_companies = placement_info.objects.filter(batch=batch,comp_id__in=companies,placement_status_id__in=[1,3])
